...
from classes import FromSource, Host, HostCategory, Htype, IsolationSyndrome, Microbe, Otype, namespaces
...
class BlazegraphSetup(object):
    ...
    def convert_categories(self):
        """
        Loads category data from JSON into the rdflib.Graph()

        """
        categories = self.import_json("data/gene_categories.txt")

        for category in categories:
            name = category[0]
            self.g.add((namespaces.n[name], namespaces.rdf.type, namespaces.n.category))

        del categories

    def convert_subcategories(self):
        """
        Loads sub-category data from JSON into the rdflib.Graph()

        """
        subcategories = self.import_json("data/gene_subcategories.txt")

        for subcategory in subcategories:
            name = subcategory[0]
            self.g.add((namespaces.n[name], namespaces.rdf.type, namespaces.n.subcategory))
