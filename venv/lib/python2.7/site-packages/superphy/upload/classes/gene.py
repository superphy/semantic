...
class Gene(NamedIndividual):
    ...
    def __init__(self, graph, name, **kwargs):
        ...
        **kwargs is a dictionary of optional metadata used to pass them in
        ...
        searchparam = ["name", "reference_contig", "vfo_id", "category", "sub_category", "uniprot", "contigs"]
    ...
    def rdf(self):
        ...
        self.graph.add((n[self.name], n.is_gene_of, gfvo.gene))
        self.graph.add((n[self.name], rdf.type, gfvo.gene))

    def reference_contig(self, reference_contig):
        """
        Adds a relationship between the gene and the contig it is referenced from.

        Args:
            reference_contig: an accession number for the genome's URI
        """
        #node = _sparql.find_genome(item)
        self.graph.add((n[self.name], n.referenced_from, n[reference_contig]))
...
class GeneLocation(NamedIndividual):
    ...
    def __init__(self, graph, name, gene, contig, begin, end, is_ref_gene):
        """
