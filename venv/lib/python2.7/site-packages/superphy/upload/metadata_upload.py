...
from classes import PendingGenome, Gene
...
class GeneMetaDataUploader(MetadataUploader):
	"""A class for parsing and uploading gene metadata from a specially prepared JSON file.
	...
	def parse_metadata(self):
		"""Takes in data from an ijson stream and parses for genetic metadata based on attached tags
		...
		with open(generate_path(self.filename), "r") as results:
			content = results.readlines()
			previous_gene = ""
			metadata = None
			contigs = []
			for line in content:
				if "LocusName" in line:
					continue
				else:
					split_line = line.split("	") #split on tabs
					gene_name = split_line[1].split("|")[0]

					if gene_name != previous_gene:
						if metadata:
							self.add_to_graph(metadata) #uploads the gene information for previous gene
						metadata = GeneMetadata(gene_name)
						previous_gene = gene_name #updating to new gene
						metadata.dict["contigs"] = contigs
						contigs = [] #resetting contigs

						# Check to see if gene has already had its reference gene added

					presence = int(split_line[3])
					if presence > 0:
						contig = split_line[6].split("|")[3].split(".")[0]
						if "complete" in split_line[6]:
...
# 	# For gene testing
# 	gmd = GeneMetaDataUploader('samples/pan_genome.txt')
# 	gmd.parse_metadata()