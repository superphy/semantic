...
from _sparql import check_NamedIndividual, is_ref_gene
				...
				for alignment in blast_record.alignments[:1]:
					for hsp in alignment.hsps:
						if hsp.expect < E_VALUE_THRESH and hsp.gaps == 0 and int(hsp.score) == hsp.identities:
							...
							if (not check_NamedIndividual(contig_accession)) and (contig_accession not in nonuploaded_genomes):
								nonuploaded_genomes.append((contig_accession, contig_accession))
							metadata = GeneMetadata(gene_name)

							location = GeneLocation()
							metadata.dict["begin"] = hsp.sbjct_start
							metadata.dict["end"] = str(int(hsp.sbjct_start) + int(hsp.score) - 1)
							contig_accession = alignment.title.split("|")[3].split(".")[0]
							metadata.dict["reference_contig"] = self.complete_genome(contig_accession, alignment.title.split("|")[4])
